...
class Good(models.Model):
    ...
    partNumber_once = models.CharField(blank = True, verbose_name = '一次性料號', max_length = 13)
    partNumber_old = models.CharField(blank = True, verbose_name = '舊料號', max_length = 13)
...
class GoodRequisition(models.Model):
    #在 `ForeignKey` 的狀況中，Django 預設會用 model 的名稱後面加 `_set` 來當作 reverse relation 的名稱，所以 `GoodRequisition.good` 的預設 reverse relation key 會是 `Good.goodrequisition_set` 全小寫。
